...
This vignette dives into the `trackSpp` function, the main 'workhorse'
function in the plantTracker R package. `trackSpp` transforms a data set
of annual maps of plant occurrence into a demographic data set. To
...
Workflow](Suggested_plantTracker_Workflow.html), Parts 1.1, 1.2, and 2.
In this example, we'll use a smaller subset of the 'grasslandData' and
'grasslandInventory' data sets that are included in plantTracker.
...
This following figures show data for one year (1922) and one species (*Heteropogon contortus*). 

```{r echo = FALSE,  fig.show = 'hold', fig.width=7, fig.align = 'left', fig.cap = "**Figure 3.2**: The value of 'buffGenet' used in the `trackSpp` function can make a big difference in genetID assignments. These examples move from no genet grouping on the left, where every polygon has its own genetID, to grouping any ramets together that are less than 10 cm apart on the right. Colors and numbers indicate different genetIDs. Buffers are drawn around ramets that belong to the same genet.", warning=FALSE}
...
figDat_2AG <- aggregate(x = figDat_2, 
          ...
          FUN = mean,
          do_union = TRUE) 
figDat_2AG <- st_buffer(figDat_2AG[,c("Species", "genetID","buffGenet","geometry")],.05)
...
ggplot(figDat) +
  geom_sf(data = figDat[figDat$Species == "grouped",], aes(fill = genetID, alpha = .2), lty = 3) +
  ...
  scale_fill_discrete(aes(), guide = FALSE) +
  theme(axis.line = element_blank(), 
        plot.margin = margin(1,0,1,0)) 
```
...
'recruit' columns. If the first year for which there is data in `dat` is actually the very first year the quadrat was sampled
(e.g. there are *Heteropogon contortus* observations in 1922, and the
quadrat SG2 was first sampled in 1922), then we put an 'NA' in both the
'age' and 'recruitment' columns. Because there was no data collected in
the previous year, we don't know if any of these plants are new
recruits, and don't know their age.
...
### *3.4* Comparing sequential years of data to track individuals through time
...
iterates through year by the index *i*. The 'current' year is the year
with the index 'i-1' in the `inv` vector, and the associated data is
stored in the `tempPreviousYear` data.frame. The 'next' year is the year
...
| then we don't know if the observations in                  | *3.     |
| `tempPreviousYear` survived or grew. They get an 'NA' in   | 4.10*** |
...
| inv[i-1] (`tempPreviousYear`) to data from year inv[i]     | *3.     |
| (`tempCurrentYear`)                                        | 4.2*.** |
| ...........                                                |         |
| .......................................................... |         |
+------------------------------------------------------------+---------+
...
exceed 'dorm', we can get data from the 'next' year (`inv[i]`). We do
this by subsetting `dat` for all observations in year `inv[i]`.
Then, we use `isClonal` to group closely-grouped polygons into genets if
...
+===========================================================+==========+
| ... is data in                                            |**Proceed |
| `tempPreviousYear`.........................               | to step  |
| ......................................................... |*3.4.4*** |
+-----------------------------------------------------------+----------+
| ... is *not* data in                                      | **Go to  |
| `tempPreviousYear`....................                    | step     |
| ......................................................... |*3.4.11***|
+-----------------------------------------------------------+----------+
...
data is stored in the `tempCurrentBuff` data.frame. This buffer is of
...
```{r echo = FALSE, warning= FALSE, fig.width=6, fig.align = 'left', fig.cap = "**Figure 3.4.4.1**: With a 10 cm buffer, these polygons in 1922 and 1923 overlap and will be identified by trackSpp() as the *same* individual and receive the same trackID."}
...
```
```{r echo = FALSE, warning= FALSE, fig.width=6, fig.align = 'left', fig.cap = "**Figure 3.4.4.2**: With a 5 cm buffer, these polygons in 1922 and 1923 overlap and will be identified by trackSpp() as the *same* individual and receive the same trackID."}
exampleDat <- grasslandData[grasslandData$Site == "AZ" & 
                              grasslandData$Quad == "SG2" , ]
exampleDatIDsTemp <- exampleDat[exampleDat$Species == "Bouteloua rothrockii" & 
...
| If there ...                                 |                       |
+==============================================+=======================+
| ... is data in                               | **If 'clonal' = TRUE, |
| `tempCurrentYear`..............              | proceed to step       |
| ............................................ | *3.4.7*.** **If       |
|                                              | 'clonal' = FALSE, go  |
|                                              | to step *3.4.8***     |
+----------------------------------------------+-----------------------+
| ... is *not* data in                         | **Take the entire     |
| `tempCurrentYear`........                    | `tempPreviousYear`    |
| ............................................ | data.frame to step    |
|                                              | *3.4.6***             |
+----------------------------------------------+-----------------------+
...
### *3.4.7* Group the `tempCurrentYear` observations into genets

If 'clonal' = TRUE for this species, use the `ifClonal` to group
individual polygons that are close enough to one another to be
considered the same genet (i.e. are within (2 x 'buffGenet') of one
another). Individuals in the same genet are given the same genetID.

### *3.4.8* Are there any overlaps between polygons in `tempPreviousYear` and `tempCurrentYear`?
...
### *3.4.9* Compare the overlap between `tempPreviousYear` and `tempCurrentYear` to assign trackIDs.

The `st_intersection` function used in step *3.4.8* returns a matrix
that gives the total area of overlap between each genet in
`tempPreviousYear` and each genet in `tempCurrentYear`. Because the
'parent' polygons (those in `tempPreviousYear`) and the 'child' polygons
(those in `tempCurrentYear`) have already been grouped by genet, each
'parent' can only have one 'child.' The `assign` function uses a while
loop to look through the matrix generated by step 3.4.8. The highest
...
```{r echo = FALSE, fig.width=7, fig.align = 'left', fig.cap = "**Figure 3.4.9.1**: Here are the data for *Heteropogon contortus* in 1922 and 1923. A 5 cm buffer is shown around each genet in 1922. Data from both years have been grouped by genet using 'buffGenet' = .01", warning = FALSE}
...
```{r echo = FALSE, fig.width=4.5, fig.align = 'left', fig.cap = "**Figure 3.4.9.2**: Here are the buffered data for *Heteropogon contortus* from 1922, overlapped with the data from 1923.", warning = FALSE}
ggplot(data = exampleSmall) +
  geom_sf(data = exampleSmall[exampleSmall$buff == "yes",], aes(color = as.factor(Year), fill = as.factor(Year)), alpha = .2, lty = 3) +
...
```{r echo = FALSE, fig.width=7, fig.align = 'left', fig.cap = "**Figure 3.4.9.3**: Here are the trackID assignments for these two years of data. Each trackID has a different color and a different number.", warning = FALSE}

exampleSmall$Site <- "AZs"
exampleSmall$Quad <- "SG2"
exampleOut <- PlantTracker::trackSpp(dat = exampleSmall[exampleSmall$buff == "no",], inv = list("SG2" = c(1922, 1923)), buff = .05, clonal = TRUE, dorm = 1, buffGenet = .01, aggregateByGenet = TRUE)
...
have a parent.
```{r echo = FALSE, fig.width=7, fig.align = 'left', fig.cap = "**Figure 3.4.9.4**: Here is a visualization of how the observations are broken into 'parents', 'ghosts', 'children', and 'orphans'.", warning = FALSE}
...
exampleOut[exampleOut$Year == 1923 & 
             exampleOut$recruit == 0, "status"] <- "children"
exampleOut[exampleOut$Year == 1923 & 
             exampleOut$recruit == 1, "status"] <- "orphan"
...
put into the 'size_tplus1' column. Then, **the `parents` data.frame is
sent to step *3.4.10*** All of the observations in the `children`
...
'age' column. However if the `orphans` occur after a gap in sampling, they instead get an 'NA' in the 'recruit' and 'age' columns, since we don't know whether they were recruited in year *i* or during the gap. Then, **both the `children` and `orphans` data.frames are sent to step *3.4.11*.**

### *3.4.10* Store the resulting demographic data
...
year of sampling, then proceed to step *3.4.11*. If `inv[i]` *is* the
last year of sampling, then the for loop is over!

### *3.4.11* Get ready for the next iteration of the loop
...
## *4.* Preparing the `trackSpp` results to be returned!
...
```{r echo = FALSE, fig.width=7, fig.align = 'left', fig.cap = "**Figure 5.1**: Here are the trackID assignments over 4 years of data. Each trackID is shown as a different.", warning = FALSE}
...
## get trackIDs 
exampleOut <- suppressMessages(PlantTracker::trackSpp(dat = exampleSmall, inv = list("SG2" = c(1922:1925)), buff = .05, clonal = TRUE, dorm = 1, buffGenet = .01, aggregateByGenet = TRUE, printMessages = FALSE))
