...
vignette: >
  %\VignetteIndexEntry{Using_the_plantTracker_'trackSpp()'_function}
...
The susbset of `grasslandData` will be referred to as `dat`, because it is the 
...
This following figures show data for one year (1922) and one species 
...
| then we don't know if the observations in                  |[3.4.12](#3_4_12)|
...
dead. **Take these observations to step [3.4.12](#3_4_12).**
...
ghosts' are added to the output data.frame. **The 'ghosts' are saved for the next step, which is [3.4.13](#3_4_13)**
...
The `st_intersection` function used in step **[3.4.8](#3_4_8)** returns a matrix
...
### *3.4.10* Flag any suspect observations
If the 'flagSuspects' argument is set to `FALSE`, proceed directly to 
step **[3.4.11](#3_4_11)**. 
If the 'flagSuspects' argument is set to `TRUE`, the following checks take 
place. The first check identifies and flags any individuals in the previous year 
that became substantially smaller in the current year. For example, there is are 
...
`tempPreviousYear` (parent) data frames go to step **[3.4.11](#3_4_11)**. 
...
### *3.4.11* Separate the 'ghosts' and the new recruits from the parent and child data frames 
...
put into the 'size_tplus1' column. Then, **the `parents` data frame is sent to step [3.4.12](#3_4_12)** All of the observations in the `children`
...
**`children` and `orphans` data.frames are sent to step [3.4.13](#3_4_13).**


<a id="3_4_12"></a>

### *3.4.12* Store the resulting demographic data 
...
year of sampling, then proceed to step **[3.4.13](#3_4_13)**. If `inv[i]` *is* 
the last year of sampling, then the for loop is over!

<a id="3_4_13"></a>

### *3.4.13* Get ready for the next iteration of the loop  
