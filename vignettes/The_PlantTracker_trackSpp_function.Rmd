...
dat, inv, dorm , buff , buffGenet , clonal,
                     species = "Species",
                     site = "Site",
                     quad = "Quad",
                     year = "Year",
...
+--------------+--------------------------------------+---------+--------+
| buffGenet    | A single value greater than or equal | Yes, but|  N/A   |
|              | to zero indicating how close polygons| only if |        |
|              | must be to one another in the same   | 'clonal'|        |
|              | year to be grouped as a genet.       | = TRUE  |        |
...
| then we don't know if the observations in                  |[3.4.10](#3_4_10)|
...
```
```{r echo = FALSE, warning= FALSE, fig.width=6, fig.align = 'left', fig.cap = "Figure 3.4.4.2: With a 5 cm buffer, these polygons in 1922 and 1923 overlap and will be identified by trackSpp() as *different* individuals and receive different trackIDs."}
exampleDat <- grasslandData[grasslandData$Site == "AZ" & 
                              grasslandData$Quad == "SG2" , ]
exampleDatIDsTemp <- exampleDat[exampleDat$Species == "Bouteloua rothrockii" & 
...
dead. **Take these observations to step [3.4.10](#3_4_10).**
...
the next step, which is [3.4.11](#3_4_11)**
...
another). Individuals in the same genet are given the same genetID. **Go to step [3.4.8](#3_4_8).**
...
The `st_intersection` function used in step [3.4.8](#3.4.8) returns a matrix
...
sent to step [3.4.10](#3_4_10)** All of the observations in the `children`
...
**`children` and `orphans` data.frames are sent to step [3.4.11](#3_4_11).**
...
year of sampling, then proceed to step [3.4.11](#3_4_11). If `inv[i]` *is* the
